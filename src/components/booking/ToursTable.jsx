...
import { TOUR_TYPES, emptyTour, formatNumber } from './helpers'

export default function ToursTable({ rows, onChange }) {
  const update = (i, patch) => {
    const next = [...rows]
    next[i] = { ...next[i], ...patch }
    onChange(next)
  }
  const add    = () => onChange([...rows, emptyTour()])
        ...
        <table className="w-full text-sm whitespace-nowrap">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-slate-600 font-medium">
              ...
              <th className="px-2 py-2 min-w-[180px]">รายการทัวร์</th>
              <th className="px-2 py-2 w-20">จำนวน</th>
              <th className="px-2 py-2 min-w-[160px]">สถานที่รับ</th>
              <th className="px-2 py-2 w-28">เวลานัด</th>
              <th className="px-2 py-2 w-28">Supplier</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.length === 0 ? (
              <tr><td colSpan="13" className="px-4 py-6 text-center font-normal text-slate-400">ยังไม่มีรายการทัวร์</td></tr>
            ) : rows.map((r, i) => (
              <tr key={i} className="hover:bg-slate-50">
                ...
                <td className="px-2 py-1">
                  <Autocomplete
                    ...
                    value={r.tour_name || ''}
                    displayKey="name"
                    onChange={(v) => update(i, { tour_name: v })}
                    onSelect={(it) => update(i, { tour_id: it.id, tour_name: it.name })}
                ...
                <td className="px-2 py-1">
                  <Autocomplete
                    ...
                    displayKey="name"
                    onChange={(v) => update(i, { pickup_location: v })}
                    onSelect={(it) => update(i, { pickup_location: it.name })}
                    inputClassName={cellInput}
                  />
                </td>
                <td className="px-2 py-1">
                ...
                <td className="px-2 py-1">
                  <select value={r.tour_type || 'option'} onChange={(e) => update(i, { tour_type: e.target.value })} className={cellSelect}>
