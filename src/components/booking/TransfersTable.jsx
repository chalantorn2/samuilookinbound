...
import { TRANSFER_SERVICES, emptyTransfer, formatNumber } from './helpers'

export default function TransfersTable({ rows, onChange, onAddBoat }) {
  const update = (i, patch) => {
    const next = [...rows]
    next[i] = { ...next[i], ...patch }
    onChange(next)
  }
  const add    = () => onChange([...rows, emptyTransfer()])
  const remove = (i) => onChange(rows.filter((_, idx) => idx !== i))

  const totalNet  = rows.reduce((s, r) => s + (Number(r.net_per_car) || 0) * Math.max(1, Number(r.vehicle_count) || 1), 0)
    ...
    <div>
        ...
        <table className="w-full text-sm whitespace-nowrap">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-slate-600 font-medium">
              ...
              <th className="px-2 py-2 min-w-[160px]">From</th>
              <th className="px-2 py-2 min-w-[160px]">To</th>
              <th className="px-2 py-2 w-28">เวลานัดรับ</th>
              <th className="px-2 py-2 w-28">จัดโดย</th>
              <th className="px-2 py-2 w-24">N/Car</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.length === 0 ? (
              <tr><td colSpan="12" className="px-4 py-6 text-center font-normal text-slate-400">ยังไม่มีรายการรับส่ง</td></tr>
            ) : rows.map((r, i) => (
              <tr key={i} className="hover:bg-slate-50">
                ...
                <td className="px-2 py-1">
                  <select value={r.service_type || 'Transfer'} onChange={(e) => update(i, { service_type: e.target.value })} className={cellSelect}>
                    {TRANSFER_SERVICES.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </td>
                <td className="px-2 py-1">
                  <input type="text" inputMode="numeric" value={r.vehicle_count ?? ''} onChange={(e) => { const v = e.target.value; if (v === '' || /^\d+$/.test(v)) update(i, { vehicle_count: v }) }} className={cellInput + ' text-center'} />
                </td>
                <td className="px-2 py-1">
                  <Autocomplete
                    endpoint="/places.php"
                ...
                <td className="px-2 py-1">
                  <TimeInput value={r.pickup_time} onChange={(v) => update(i, { pickup_time: v })} className={cellInput} />
                </td>
                <td className="px-2 py-1">
                  <Autocomplete
                ...
                <td className="px-2 py-1">
                  <MoneyInput value={r.net_per_car} onChange={(v) => update(i, { net_per_car: v })} className={cellInput + ' text-right'} />
                </td>
                <td className="px-2 py-1">
                  <MoneyInput value={r.sale_adult}  onChange={(v) => update(i, { sale_adult:  v })} className={cellInput + ' text-right'} />
                </td>
                <td className="px-2 py-1">
                  <MoneyInput value={r.sale_child}  onChange={(v) => update(i, { sale_child:  v })} className={cellInput + ' text-right'} />
                </td>
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          {onAddBoat && (
            <button type="button" onClick={onAddBoat} className={boatBtn}>+ จองเรือ</button>
          )}
        </div>
...
const boatBtn = "bg-sky-500 hover:bg-sky-600 text-white text-sm font-medium px-3 py-1.5 rounded transition"
